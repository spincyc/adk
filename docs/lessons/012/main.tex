...
it is the acquisition indicator. Initialization is transactional: a failed
...
all-red best-effort and performs canonical shutdown. No call blocks while a
phase elapses.
...
Engine fault requests all-red & use the host fault fixture; do not create
shorts on the powered circuit & the snapshot commands two reds and pedestrian
stop; adapter failure only makes a best-effort request before shutdown\\
...
Record all three independently. Software cannot prove that a lamp or wire
actually followed its command; only observation can.}
...
\begin{itemize}
  ...
  \item[\checkbox] A healthy output path presents the requested running fault
        indication: all-red plus pedestrian stop.
  \item[\checkbox] Injected adapter failure attempts every safe-state write,
        then performs canonical reverse-order shutdown.
