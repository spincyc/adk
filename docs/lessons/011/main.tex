...
Evidence & nine LEDs, button, named lamp test points & Status & host verified; bench open\\
...
Fault & both vehicle reds, pedestrian stop & latched until reinitialize\\
\bottomrule
\end{tabularx}
...
timestamps, and input events produces the same snapshots.
...
condition. Only \texttt{initialize()} begins a new run. The adapter presents
the all-red fault signals and turns D13 off. It treats an output-write failure
more conservatively: every \texttt{MonoLed} shuts down, making each owned output
high impedance.
...
\textbf{Current status: host verified; Mega 2560 firmware and physical bench
acceptance remain open until their command output and completed record are
published.}
