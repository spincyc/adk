...
Equal sequence and observation time are allowed only when the complete
envelope is byte-identical to the accepted envelope. That duplicate is
idempotent. The same byte-identical envelope at a valid forward policy time is
also idempotent: it neither ages the published snapshot nor extends a radiant
candidate. A changed duplicate, regression, backward policy time, exact
half-range ambiguity, or identity/revision change rejects the whole update
without changing the last observation or radiant candidate. Sequence zero is
invalid.
