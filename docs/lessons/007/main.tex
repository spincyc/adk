...
potentiometer, ordinary LED, measured 220\,\(\Omega\) resistor, jumpers, and
multimeter; logic analyzer or oscilloscope optional\\
...
\begin{tikzpicture}[x=1cm,y=1cm,>=Latex,
    ...
    \node[right=5mm of d13,text width=7.0cm] {
        on = all three first-class objects acquired; off = startup fault or
        stopped circuit};
\end{tikzpicture}
\end{center}
...
\texttt{setup()} acquires the diagnostic LED, input, and output. D13 turns on
only after all three initialize. Its \texttt{loop()} then reads as the circuit
does:
...
\textbf{Observe}: take exactly one explicit raw sample.
...
Use \texttt{make boards} to discover the port. This example intentionally emits
no Serial records; its first evidence is the circuit. Repository targets still
support later optional logs:
...
An empty Serial stream is expected here and is not a failure. Disconnect any
monitor before uploading if the operating system cannot share the port.
...
then stays on. \textbf{Observe}: watch D13 without touching the potentiometer.
\textbf{Interpret}: on supports the claim that the program acquired A0, D6,
and D13. It does not prove correct breadboard wiring or ADC accuracy.
...
After recording normal behavior, disconnect USB. Before rewiring, run a
purpose-built shutdown acceptance sketch or host test that calls
\texttt{shutdown()}; never short a live output to test it. Confirm D13 and the
external LED turn off. With a suitable high-impedance instrument and the
published acceptance procedure, verify A0 and D6 are no longer actively driven.
This electrical observation is separate from the earlier D13 acquisition
signal.
...
\midrule
D13 stays off & remove power; inspect pin choices and unpowered continuity &
...
\midrule
Resources acquired & D13 turns on after reset & & \\
...
\begin{enumerate}
    ...
    \item Add optional Serial records without changing the circuit-native
    acceptance criteria. Log raw and duty only when either changes by a useful
    threshold.
