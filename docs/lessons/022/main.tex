...
\begin{center}
build the input \(\rightarrow\) see pulse length \(\rightarrow\) count eight
records\\
\(\downarrow\)\\
see the software restart reward \(\rightarrow\) see the fixed safe ending
\end{center}
...
two moisture-scale bytes, one sample-state byte, and one synthetic metadata
