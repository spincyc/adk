...
the fixed flat/package reference. If the specimen differs,
...
short trace, the ``filtered'' column is supplied so the state-machine
...
\section{Prepared workflow}


The circuit supplies its evidence without a terminal. The optional telemetry
build emits only \texttt{raw=} ADC counts,
...
learner project does not include a bench record.
\section{Use with}

Use the maintained HTML lesson for searchable API links, errata, source
downloads and maintained procedures. Use this PDF for predictions, schematics,
and evidence interpretation. The canonical editable sketch
