...
\begin{tikzpicture}[
  ...
  \node[box] (fixtures) {owned fixtures\\packets};
  \node[box,right=of fixtures] (track) {sequence +\\freshness};
  \node[box,right=of track] (console) {console\\decision};
  \node[box,above right=of console] (display) {selected source\\presentation};
  \node[box,right=of console] (health) {RGB health\\cadence};
  \node[box,below right=of console] (record) {bounded record\\acceptance LED};
  \draw (fixtures) -- (track);
  \draw (track) -- (console);
...
optional character display, next and acknowledge buttons, RGB source-health
evidence, record LED, and separate paths from console decision to presentation
and record acceptance. No radio or actuator is shown.}
\end{center}
