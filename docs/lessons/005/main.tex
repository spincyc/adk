...
returns \texttt{Status::InvalidArgument} without starting a new tone.
\texttt{play()} before initialization returns \texttt{Status::NotInitialized}.
...
\midrule
D6 pin owner & \texttt{Status::ResourceBusy} & no pin mode or tone call\\
Timer2 owner & \texttt{Status::ResourceBusy} & D6 claim rolled back; no hardware call\\
Unrelated pin/timer owner & \texttt{Status::Ok} & D6 and Timer2 claimed\\
Same initialized sounder & \texttt{Status::Ok} & no repeated hardware work\\
\bottomrule
\end{tabularx}
\end{center}
...
require (sounder.initialize () == adk::Status::Ok);
...
\begin{itemize}
    ...
    \item[\checkbox] Initialization returns \texttt{Status::Ok}.
