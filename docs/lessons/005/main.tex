...
transducer, one 100\,$\Omega$ series resistor, one measured
10\,k\(\Omega\) weak-pull resistor, a high-impedance oscilloscope or logic
analyzer, a DMM, and jumper wires. Use the
...
Acquisition evidence & both components owned & D13 on 250\,ms, then off 750\,ms\\
...
returns a status whose \texttt{error()} is
\texttt{StatusCode::}\allowbreak\texttt{InvalidArgument} without starting a new
tone.
\texttt{play()} before initialization reports \texttt{error()} as
\texttt{StatusCode::}\allowbreak\texttt{NotInitialized}.
...
The canonical sketch is finite and gives acquisition its own evidence. After
both components initialize, TP-ACQ/D13 is lit for 250\,ms and dark for 750\,ms
while the sounder remains silent. It then plays one 440\,Hz, 250\,ms cue and
calls \texttt{shutdown()} on the D13 LED and sounder. It makes no further
hardware calls. Pressing reset repeats the entire sequence. Thus the D13 pulse
proves acquisition separately from the cue at TP-SND/D6 and from D6's released
safe state.

\begin{enumerate}
    \item With USB removed, identify the exact board, passive piezo, resistor,
          instruments, and probe input impedance. Label the D6 pin-side end of
          the 100\,$\Omega$ resistor \textbf{TP-SND/D6}; label the factory LED
          signal \textbf{TP-ACQ/D13}. Trace the circuit and verify no short
          between 5\,V and GND.
    \item From the exact retained board revision's schematic and board
          inspection, draw the complete D13 factory-LED branch: PB7/D13,
          resistor value, LED polarity and electrical limits, and any buffer
          or other load between the MCU pin and indicator. Calculate its
          worst-case ON current from the measured rail and documented component
          limits. Include that branch in the per-pin, grouped-pin, and
          whole-device budget; ``built in'' does not mean load-free.
    \item Calculate the worst-case D6 edge current from the identified
          transducer's capacitance/impedance, its primary source, the measured
          resistor, and the Mega output limits. Record the method and margin;
          the resistor alone is not an identity or current guarantee.
    \item Attach the high-impedance waveform probe to TP-SND/D6 and its
          reference to Mega GND while unpowered. Connect USB, upload, and
          observe D13's 250\,ms pulse and 750\,ms dark separator before the cue.
    \item At TP-SND/D6, record the cue's high/low levels, frequency, and finite
          duration. Interpret the waveform and audible cue together; hearing a
          sound alone does not establish frequency, duration, or pin state.
    \item Wait for controlled shutdown. Host trace evidence must independently
          end with tone stop, D6 \texttt{INPUT}, D13 LOW, and D13
          \texttt{INPUT}. Darkness and silence do not prove high impedance.
    \item To prove D6's physical safe state, remove USB and install the measured
          10\,k\(\Omega\) weak pull-up from the measured 5\,V rail to
          TP-SND/D6. Include probe/DMM input resistance, transducer leakage,
          and Mega leakage in the predicted released-voltage range. Reconnect
          USB, record TP-SND/D6 during the driven cue and after shutdown, then
          remove USB before removing the fixture. A released voltage in the
          calculated range is the physical high-impedance evidence.
    \item Prove D13's physical post-shutdown state with a \emph{separate},
          board-specific weak-bias calculation. Using the identified factory
          branch, meter/probe impedance, MCU leakage limits, rail uncertainty,
          and component tolerances, choose and measure a temporary resistor
          from TP-ACQ/D13 to the rail that makes the predicted driven-LOW and
          released-\texttt{INPUT} voltage ranges non-overlapping. The fixture
          must remain within every D13 and factory-branch current limit even if
          firmware drives the opposite level. Install it only while USB is
          removed; reconnect USB, record D13 during the dark separator and
          after controlled shutdown, then remove USB before removing the
          fixture. If no safe resistor yields separable ranges for that board
          branch, do not claim physical high impedance; use a separately
          reviewed high-impedance current/impedance method instead.
    \item With all fixtures removed, reconnect USB and press reset once. Confirm
          the acquisition pulse, separator, cue, and shutdown all repeat.
    \item Disconnect USB at the computer end. Measure the 5\,V rail,
          TP-ACQ/D13, and TP-SND/D6 relative to GND. Record the power-removed
          ranges before touching or dismantling the circuit.
\end{enumerate}

Stop immediately and remove USB for an unidentified or warm part, unexpected
continuous sound, a waveform outside the identified part or Mega limits,
current above either calculated limit, overlapping weak-bias prediction ranges,
a failed acquisition separator, or any failure to reach shutdown within
2 seconds after reset. Do not move a probe or change a weak-bias fixture while
powered.
...
\textbf{Physical work remains pending.} Leave every field blank until the named
observation is made on the retained bench assembly.

Date/time: \blank{1.5in}\quad Operator: \blank{1.5in}\\
Reviewer/date: \blank{1.8in}\\
Board maker/model/revision/serial or asset ID: \blank{3.4in}\\
Port/FQBN: \blank{2.3in}\quad ADK/sketch commit: \blank{2.3in}\\
\texttt{arduino-cli}/AVR core versions: \blank{3.5in}\\
Upload command: \blank{4.8in}\\
USB source/cable/current limit and removal point: \blank{3.3in}\\
DMM model/range/input resistance: \blank{2.9in}\\
Scope/analyzer model, probe, attenuation, bandwidth, and reference:
\blank{2.5in}\\
Exact board schematic/revision and D13 branch topology:
\blank{3.0in}\\
D13 resistor/LED/buffer identities, values, tolerances, and sources:
\blank{2.2in}\\
Calculated D13 branch current/method/margin: \blank{3.0in}\\
Piezo maker/part/markings/passive-identity evidence:
\blank{3.2in}\\
Piezo rated voltage and capacitance/impedance: \blank{2.4in}\\
Primary source and revision: \blank{3.8in}\\
100\,\(\Omega\) measured value/tolerance/rating: \blank{2.5in}\\
10\,k\(\Omega\) fixture measured value/tolerance/rating:
\blank{2.4in}\\
Measured 5\,V rail: \blank{1.3in}\\
Calculated worst-case D6 current/method/margin: \blank{3.1in}\\
Mega per-pin, grouped-pin, and total-device limits checked: \checkbox\\
Flash: \blank{0.8in} bytes\quad SRAM: \blank{0.8in} bytes

\begin{center}
\begin{tabularx}{\linewidth}{@{}p{0.18\linewidth}XXX@{}}
\toprule
Evidence & Predict & Observe & Interpret\\
\midrule
Acquisition &
D13 pulse/separator; sounder silent &
\rule{0pt}{3.5ex} &
\\[9ex]
Primary cue at TP-SND/D6 &
levels, 440\,Hz, 250\,ms &
&
\\[9ex]
Current/limits &
D6 edge and D13 branch margins &
&
\\[9ex]
Controlled shutdown &
tone stop; D6 and D13 released &
&
\\[9ex]
Weak-pull D6 safe state &
driven/released voltage ranges &
&
\\[9ex]
D13 safe state &
board-specific bias ranges &
&
\\[9ex]
Reset &
...
Unpowered continuity map and TP labels confirmed: \checkbox\quad
Initialization status recorded independently: \checkbox\\
Host trace records tone stop/D6 input and D13 LOW/input: \checkbox\quad
D6 weak pull installed and removed only while unpowered: \checkbox\\
D13 fixture resistor/value/rail: \blank{2.5in}\quad
opposite-drive worst-case current safe: \checkbox\\
D13 driven-LOW/released predicted ranges: \blank{2.6in}\\
D13 driven-LOW/released observed values: \blank{2.8in}\\
D13 fixture installed and removed only while unpowered: \checkbox\\
Reset completed: \checkbox\quad USB physically removed before handling:
\checkbox\\
Deviation and stop action: \blank{4.2in}\\
Remaining uncertainty: \blank{4.5in}\\
