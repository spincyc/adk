...
\begin{tikzpicture}[
    ...
    >=latex]
  \node[box] (buttons) {4 \texttt{Button}s\\D22--D25};
  \node[box,right=of buttons] (adapter) {input adapter\\three masks};
  \node[box,right=of adapter] (simon) {\texttt{Simon}\\pure game state};
  \node[box,below=of simon] (snapshot) {\texttt{SimonSnapshot}\\one coherent frame};
  \node[box,left=of snapshot] (cueled) {4 \texttt{MonoLed}s\\D30--D33};
  \node[box,left=of cueled] (feedback) {RGB D5--D7\\piezo D11};
  \node[box,above=of simon] (source) {\texttt{CueSource}\\fixed or XorShift32V1};
  \draw[->] (buttons) -- (adapter);
  \draw[->] (adapter) -- (simon);
  \draw[->] (source) -- (simon);
  \draw[->] (simon) -- (snapshot);
  \draw[->] (snapshot) -- (cueled);
  \draw[->] (snapshot.south) |- (feedback.east);
\end{tikzpicture}
\end{center}
...
four-button/RGB/sound hardware adapter remains experimental until its Mega
...
\begin{tikzpicture}[
    ...
    >=latex]
  \node[state] (idle) {Idle};
  \node[state,right=of idle] (on) {PlaybackOn};
  \node[state,right=of on] (gap) {PlaybackGap};
  \node[state,below=of gap] (press) {AwaitPress};
  \node[state,left=of press] (release) {AwaitRelease};
  \node[state,left=of release] (round) {RoundSuccess};
  \node[state,below=of release] (success) {GameSuccess};
  \node[state,right=of success] (failure) {GameFailure};
  \draw[->] (idle) -- node[above]{start event} (on);
  \draw[->] (on) -- node[above]{cue duration} (gap);
  \draw[->] (gap) to[bend left=25] node[above]{next cue} (on);
  \draw[->] (gap) -- node[right]{playback done} (press);
  \draw[->] (press) -- node[above]{correct single press} (release);
  \draw[->] (release) -- node[above]{more input} (press);
  \draw[->] (release) -- node[above]{round done} (round);
  \draw[->] (round) to[bend right=20] node[above]{grow + replay} (on);
  \draw[->] (release) -- node[left]{maximum reached} (success);
  \draw[->] (press) -- node[right]{bad / timeout} (failure);
  \draw[->] (success) to[bend right=24] node[below]{start resets} (on);
  \draw[->] (failure) to[bend left=20] node[below]{start resets} (on);
\end{tikzpicture}
\end{center}
...
\warningbox{Physical acceptance is pending. Do not fill an observation from a
host trace, successful compile, expected LED state, or this worksheet. A named
operator and independent reviewer must observe the retained specimens on the
identified bench.}

\begin{enumerate}
  \item Record repository commit, Arduino CLI and AVR core versions, operator,
        reviewer, exact board, retained specimens, primary sources, supply and
        current limit, meter, and logic-analyzer model if used.
  \item Disconnect power. Build one branch at a time from the exact schematics.
        Verify seven 330\(\Omega\) resistors, RGB common cathode, four button
        contact pairs, passive piezo, and common ground.
  \item Compile the supported Simon engine and adapter example for
        \texttt{arduino:avr:mega} with the fixed-replay fixture enabled, using
        the exact command in the golden-replay section. Record the define,
        flash, and static RAM.
  \item Reset that artifact, whose bounded source is exactly One, Three.
        Predict D13 off before
        acquisition, on for 250\,ms, then off through the 750\,ms separator.
        Observe it separately
        from every cue, RGB, and sound behavior.
  \item Verify each button's raw and debounced diagnostic state separately.
        Then verify cue-to-button mapping One through Four.
  \item Play the golden two-cue game. Observe cue LED, RGB state, tone, and
        serial snapshot at every transition.
  \item Press two buttons as nearly simultaneously as practical and record only
        a physical chord observation. Use the two-bit host replay---not hand
        timing---for the exact-simultaneity claim.
  \item Verify mismatch, timeout, round success, game success, terminal restart,
        and a button held through reset.
  \item Run the same recorded seed twice. Compare the displayed sequence, not
        just the final score.
  \item Arrange separate runs so the 120-second deadline expires during
        PlaybackOn, AwaitPress, and sound. At each expiry, measure all behavior
        outputs and D13 inactive throughout the 250 ms interval, then confirm
        shutdown releases the pins and the loop remains halted. Reset between
        trials and record the expected reacquisition indication.
  \item Prove high impedance on D5--D7, D11, D13, and D30--D33 with a reviewed
        10\,k\(\Omega\)/10\,k\(\Omega\) midpoint fixture installed on one pin
        at a time while unpowered. Calculate driven and released ranges from
        measured rail, resistor tolerance, and DMM input resistance. Record D6
        separately. Never move the fixture while powered.
  \item Remove USB and every other source. Verify the rail is de-energized
        before teardown.
\end{enumerate}

\subsection{Identity, configuration, and review card}
...
\midrule
Commit / sketch & & Operator / date &\\[7pt]
Independent reviewer / date & & Result: pass / fail / open &\\[7pt]
Mega make / model / revision / ID & & USB supply / measured rail / limit &\\[7pt]
Board schematic / D13 branch & & D13 predicted / measured load &\\[7pt]
CLI / AVR core / upload command / port & & Meter / analyzer / probe reference &\\[7pt]
Four switch IDs / source & & Four cue LED IDs / polarity source &\\[7pt]
RGB ID / topology / pinout source & & Piezo ID / passive rating / source &\\[7pt]
Seven measured resistors & & Series piezo resistor &\\[7pt]
Flash / static RAM & & Algorithm / normalized seed &\\[7pt]
Deviations / uncertainty & & Final power-removal check &\\[7pt]
\bottomrule
\end{tabularx}
\end{center}

\subsection{Prediction / observation / interpretation record}

\begin{center}
\small
\begin{tabularx}{\linewidth}{@{}p{0.19\linewidth}X X X@{}}
\toprule
Stage / location / timing & Prediction & Observation & Interpretation\\
\midrule
Unpowered continuity and polarity & No shorts; values and pinouts match sources & &\\[13pt]
D13 after setup & 250\,ms pulse then 750\,ms dark; game outputs inactive & &\\[13pt]
D22--D25 raw, then debounced & Each released high and pressed low; one event per press & &\\[13pt]
Fixed replay behavior & Cue/RGB/tone sequence matches published timestamps & &\\[13pt]
Near-simultaneous hand press & Physical chord only; no exact timing claim & &\\[13pt]
Reset with one button held & Reacquires resources; no press event until release/repress & &\\[13pt]
120 s deadline / 250 ms interval & Tone and all lights inactive, then reverse shutdown and halt & &\\[13pt]
Physical power removal & Rail de-energized before teardown & &\\[13pt]
\bottomrule
\end{tabularx}
\end{center}

\subsection{Current and per-pin safe-state record}

\begin{center}
\small
\begin{tabularx}{\linewidth}{@{}l X X X@{}}
\toprule
Item & Prediction / method & Observation & Interpretation\\
\midrule
Each of 7 LED branches & rail, resistor drop, \(I=V_R/R\) & &\\[12pt]
Normal commanded total & D13 + cue + RGB; compare pin/port/package limits & &\\[12pt]
Seven-die fault bound & minimum \(V_f\); compare pin/port/package limits & &\\[12pt]
Piezo D11 & identified rating and bounded command & &\\[12pt]
D5 / D6 / D7 after shutdown & 10 k\(\Omega\)/10 k\(\Omega\) midpoint trial & &\\[12pt]
D11 / D13 after shutdown & 10 k\(\Omega\)/10 k\(\Omega\) midpoint trial & &\\[12pt]
D30 / D31 / D32 / D33 after shutdown & 10 k\(\Omega\)/10 k\(\Omega\) midpoint trial & &\\[12pt]
D22 / D23 / D24 / D25 after shutdown & pulls released; midpoint trial & &\\[12pt]
\bottomrule
\end{tabularx}
\end{center}

\textbf{Stop conditions.} Stop immediately, record the incomplete stage, and
remove USB for heat, odor, reset, painful or distorted sound, unexpected
brightness, a specimen identity or pinout mismatch, measured current above the
reviewed aggregate bound, or any observation inconsistent with the predicted
safe state. Do not continue by revising the prediction after observing the
result.

\textbf{Status language.} Until this checklist is signed, report ``Simon engine
host verified; hardware adapter experimental.'' Compilation alone is not
...
Golden replay passed twice: \checkbox{}\quad
Chord rejected: \checkbox{}\quad
Current calculated: \checkbox{}\quad
Shutdown measured: \checkbox{}\quad
Power removed: \checkbox{}\quad
Independent review: \checkbox{}
