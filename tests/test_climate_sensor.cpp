...
namespace {
    ...
    struct DestructorProbe final : adk::ClimateSensor
    {
        ...
        {
            return initialized_ ? adk::Status::Ok : adk::Status::NotInitialized;
        }
    ...
    void testLifecycleAndEmptyTrace ()
    {
        ...
        assert (sensor.update (adk::TimePoint (0)) == adk::Status::NotInitialized);
        assert (sensor.initialize () == adk::Status::Ok);
        assert (sensor.initialize () == adk::Status::Ok);
        assert (sensor.initialized ());
        assert (sensor.update (adk::TimePoint (0)) == adk::Status::Ok);
    ...
    void testInvalidTraceIsRejected ()
    {
        const adk::RecordedClimateFrame backwards[] = {
            {adk::TimePoint (100), validSample (1000, 500, 100), adk::Status::Ok},
            {adk::TimePoint (99), validSample (1100, 510, 99), adk::Status::Ok}};
        adk::RecordedClimateSensor nullTrace (nullptr,   1);
        adk::RecordedClimateSensor badOrder  (backwards, 2);

        assert (nullTrace.initialize () == adk::Status::InvalidArgument);
        assert (badOrder.initialize () == adk::Status::InvalidArgument);
    }

    void testFramesAdvanceOnlyWhenDue ()
    {
        const adk::RecordedClimateFrame frames[] = {
            {adk::TimePoint (10), validSample (2100, 450, 10), adk::Status::Ok},
            {adk::TimePoint (20),
             faultSample (2100, 450, 20, adk::ClimateSampleState::ChecksumFailure),
             adk::Status::HardwareFailure},
            {adk::TimePoint (20), validSample (2200, 460, 20), adk::Status::Ok}};
        adk::RecordedClimateSensor sensor (frames, 3);

        assert (sensor.initialize () == adk::Status::Ok);
        assert (sensor.update (adk::TimePoint (9)) == adk::Status::Ok);
        assert (sensor.frameIndex () == 0);
        assert (sensor.update (adk::TimePoint (10)) == adk::Status::Ok);
        assert (sensor.frameIndex () == 1);
        assert (sensor.sample (adk::TimePoint (10), adk::Duration (10))
                    .temperatureCentiCelsius == 2100);
        assert (sensor.update (adk::TimePoint (20)) == adk::Status::Ok);
    ...
    {
        const adk::RecordedClimateFrame frames[] = {
            {adk::TimePoint (7), validSample (2000, 400, 7), adk::Status::Ok},
            {adk::TimePoint (7),
        ...
        assert (sensor.initialize () == adk::Status::Ok);
        assert (sensor.update (adk::TimePoint (7)) == adk::Status::HardwareFailure);
        ...
        assert (sensor.update (adk::TimePoint (7)) == adk::Status::Ok);
    }
    ...
    void testFaultRetainsDiagnosticValuesAndCanRecover ()
    {
        const adk::RecordedClimateFrame frames[] = {
            {adk::TimePoint (0),
             ...
             adk::Status::HardwareFailure},
            {adk::TimePoint (5), validSample (2300, 480, 5), adk::Status::Ok}};
        adk::RecordedClimateSensor sensor (frames, 2);

        assert (sensor.initialize () == adk::Status::Ok);
        assert (sensor.update (adk::TimePoint (0)) == adk::Status::HardwareFailure);
        ...
        assert (sensor.update (adk::TimePoint (5)) == adk::Status::Ok);
    ...
    void testStaleBoundaryIsInclusiveAndWrapSafe ()
    {
        ...
        assert (sensor.initialize () == adk::Status::Ok);
        assert (sensor.update (adk::TimePoint (UINT32_MAX - 2U)) == adk::Status::Ok);
    ...
    {
        const adk::RecordedClimateFrame frames[] = {
            {adk::TimePoint (10), validSample (2400, 500, 10), adk::Status::Ok}};
        adk::RecordedClimateSensor sensor (frames, 1);
        ...
        assert (sensor.update (adk::TimePoint (9)) == adk::Status::InvalidArgument);
        assert (sensor.frameIndex () == 1);
    }
    ...
    void testWrapBoundaryAndLargeValidAdvance ()
    {
        const adk::RecordedClimateFrame frames[] = {
            {adk::TimePoint (UINT32_MAX), validSample (2400, 500, UINT32_MAX),
             adk::Status::Ok},
            {adk::TimePoint (0), validSample (2500, 510, 0), adk::Status::Ok}};
        adk::RecordedClimateSensor sensor (frames, 2);

        assert (sensor.initialize () == adk::Status::Ok);
        assert (sensor.update (adk::TimePoint (UINT32_MAX - 1U)) == adk::Status::Ok);
        assert (sensor.frameIndex () == 0);
        assert (sensor.update (adk::TimePoint (UINT32_MAX)) == adk::Status::Ok);
        assert (sensor.frameIndex () == 1);
        assert (sensor.update (adk::TimePoint (0)) == adk::Status::Ok);
        assert (sensor.frameIndex () == 2);
        assert (sensor.update (adk::TimePoint (static_cast<uint32_t> (INT32_MAX))) ==
                adk::Status::Ok);
    }
    ...
    {
        const adk::RecordedClimateFrame frames[] = {
            {adk::TimePoint (1), validSample (2000, 400, 1), adk::Status::Ok},
            {adk::TimePoint (2), validSample (2100, 410, 2), adk::Status::Ok}};
        adk::RecordedClimateSensor sensor (frames, 2);

        assert (sensor.initialize () == adk::Status::Ok);
        assert (sensor.update (adk::TimePoint (2)) == adk::Status::Ok);
    ...
    void testReplayProducesIdenticalStatusAndSamples ()
    {
        const adk::RecordedClimateFrame frames[] = {
            ...
            {adk::TimePoint (3),
             ...
             adk::Status::HardwareFailure},
            {adk::TimePoint (5), validSample (2100, 410, 5), adk::Status::Ok}};
